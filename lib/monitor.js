...
function monitor (options) {
  ...
  function get_attachment_name (change, cb) {
    if (!change._rev) {
      return cb(new Error('Invalid data: expected "_rev" attribute'));
    }
    if (!change._attachments) {
      return cb(new Error('Invalid data: expected "_attachments" attribute'));
    }
    var m = change._rev.match(/^(\d+)\-([0-9a-f]{32})$/i);
    if (!m) {
      return cb(new Error('Invalid data: attribute "_rev" = "' + change._rev + '" has invalid format'));
    }
    ...
    if (change['dist-tags'] && change['dist-tags'].latest && change.versions && change.versions[change['dist-tags'].latest] && change.versions[change['dist-tags'].latest].name) {
      var expected_name = change.versions[change['dist-tags'].latest].name + '-' + change['dist-tags'].latest + '.tgz';
      if (found_attachment !== expected_name) {
        logger.info('Note: unexpected name for attachment "' + found_attachment + '" (expected "' + expected_name + '")');
      }
    }
    cb(null, found_attachment);
  }
